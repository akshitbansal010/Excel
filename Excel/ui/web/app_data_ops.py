...
def get_current_dataframe() -> Optional[pd.DataFrame]:
    ...
    return app_state.get_current_dataframe()
...
def add_filter(column: str, operator: str, value: Any):
    ...
    # Add to undo stack
    app_state.add_to_undo_stack('add_filter', {
        'column': column,
        'operator': operator,
        'value': value
    })
...
def remove_filter(index: int):
    ...
    if 0 <= index < len(st.session_state.active_filters):
        ...
        app_state.add_to_undo_stack('remove_filter', removed)
        st.session_state.page = 1
...
def get_filter_operators(column_type) -> List[str]:
    ...
    if pd.api.types.is_numeric_dtype(column_type):
        return app_config.NUMERIC_OPERATORS
    else:
        return app_config.TEXT_OPERATORS
...
def apply_sort(column: str, ascending: bool = True):
    ...
    app_state.add_to_undo_stack('sort', {
        'column': column,
        'ascending': ascending
    })
...
def add_column(column_name: str, default_value: Any = None) -> bool:
    ...
    # Add to undo stack
    app_state.add_to_undo_stack('add_column', {
        'column': column_name,
...
def delete_column(column_name: str) -> bool:
    ...
    # Add to undo stack
    app_state.add_to_undo_stack('delete_column', {
        'column': column_name,
...
def rename_column(old_name: str, new_name: str) -> bool:
    ...
    # Add to undo stack
    app_state.add_to_undo_stack('rename_column', {
        'old_name': old_name,
        'new_name': new_name
    })
...
def add_column_from_expression(expression: str, new_column_name: str) -> bool:
    ...
    try:
        ...
        # Add to undo stack
        app_state.add_to_undo_stack('add_column_sql', {
